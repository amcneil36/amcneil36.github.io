...
So, whenever we start working on a feature, we should never have anyone switch to working on a defect until we release our feature, right? Well unfortunately, it's not that simple. Notice that the example above was for a **known defect**. If we were somehow aware of all defects in production, then yes; switching to work on a defect after already having completed part of a feature would just mean that we should have done the defect first. However, we won't be aware of all the defects in production at any given time. Some of the defects won't be discovered or reported by our clients until we are in the middle of working on a feature. If this defect is more important than the feature we are working on, then at least one person needs to stop working on the feature to start fixing this defect. As a result, we will often simultaneously have at least one teammate working on a feature and one teammate working on a defect fix.
