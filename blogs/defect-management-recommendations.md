...
Defects take time and effort to fix. Any time spent fixing a defect is less time spent working on a feature. Should we stop to fix the defect now or should we keep working on the feature instead? This is a judgement call that has to be made by the customer. When comparing the defect to the feature, the customer can look at value, risk, time estimates, and other factors to determine whether the bug fix or feature is a higher priority. In some scenarios, the customer will choose to have the bug fixed now. In other scenarios, the customer will place the defect further down the product backlog and choose to have people keep working on the feature.
