...
This is a bad idea because it may cause us to not be working on the highest priority items at any point in time. If you look at the product backlog and the most important items are defects, then it is fine to have the whole team do defect fixes during a sprint. However, if no defects are identified as the highest priority items, we don't want the team to be forced to work on defects just because their team rule indicates that it is that time of the year to only do defect fixes.
