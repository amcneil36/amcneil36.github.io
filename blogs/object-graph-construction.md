...
Suppose we create all of our objects at the entry point of the application. This means that all of our other classes will be very re-usable and are going to have less logic in them since they are not concerned about creational logic. Our entry-point will have a bunch of dependencies hard-coded with new operators though. Doesn't that mean that our entry-point of the application is no longer re-usable? The entry-point wouldn't be re-usable anyway. The entry-point is usually code for what you want to happen when your server starts up. What other people want to happen when their server starts up might be completely different.
