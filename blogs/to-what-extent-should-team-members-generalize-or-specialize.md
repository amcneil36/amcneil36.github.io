...
## cons
This setup makes it difficult to remove bottlenecks.<sup>2</sup> If something is going wrong in a particular area, we might want to have more than one person help out in that area. If people are all specialists, we may not be able to have more than one person to help out so having specialists makes bottlenecks take longer to remove. Secondly, some sprints might contain works that require a little bit different skill sets than other sprints. This means that in some sprints, our specialist set up aligns well with the work that needs to be done but in other sprints the specialist set up does not align well with the work that needs to be done.<sup>3</sup> Thirdly, when a specialist is out of office, it might be hard for someone else to cover their area.

There is another large issue that arises when people only work in their specialty. One layer can start to get ahead of another layer. We can reach a point where our layer is not able to integrate with another layer because the other layer is too far behind. We might end up continuing to work on our layer, having to make assumptions about how the other layer is going to look in the future. We might find out later that this assumption was wrong. As a result, we could have to throw away the code we wrote in our layer that was written under an incorrect assumption about how the other layer would look. This is why we need to be frequently working on each layer and integrating each layer. Lastly, our layer might finish way before another layer but until all the work is done in all layers, we still can't release.
...
### Specialist vs Generalizing-Specialist: the two most important concepts that will be used to answer this question
There are two important concepts that will be used to determine the answer to the question of whether or not we want specialists or generalizing-specialists. One concept is the fact that some people may not be able to be a specialist for 40 hours per week on a team because there would not be enough work if they are only working in the area they are specialized in.<sup>4,5</sup> The second concept is that certain types of tasks are significantly more important at certain points in time and significantly less important at other points in time. Both of these concepts will strongly apply to non-software engineers. As for software engineers, the first concept will apply a medium amount but the second concept will strongly apply. Let's take a look at each one separately.  

### Specialist or Generalizing-Specialist for Non-Software Engineers?
Fleshing out requirements is only needed right before work is about to begin by others to introduce the functionality to satisfy the requirements. We want to make decisions at the last responsible moment so that we have the most information when making that decision.<sup>6</sup> When you are a specialist and only write requirements, you might end up having written some of the requirements way too early in advance as a result of not having anything else to do. Thus we would want whoever is writing requirements to be able to work in other areas during timeslots where it is not appropriate to be writing requirements. Whoever writes requirements may be able to assist in other areas too such as testing or documentation. Someone who is specialized in UX design might also need to be able to help out in other areas such as documentation or testing for the same reasons.<sup>4</sup> So for non-software engineers we clearly want generalizing-specialists.<sup>2,3,4,5,7,8</sup> For software engineers, the answer to this question is not immediately obvious because there will be much more people in that role on a team which could promote specialization a bit more. Let's see though.
