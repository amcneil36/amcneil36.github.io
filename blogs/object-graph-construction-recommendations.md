...
It could be said that there are two types of classes: factory classes and business logic classes. The bulk of our objects will be created in factories. However, there are still some objects that might make sense to create in our business logic classes. This would be classes like value objects, entities, and collections. These classes don't have any logic in them that we would need to override so programming to implementation and instantiating them in our business logic classes with the new operator is fine. 
