...
Have the factory make direct dependencies that are needed. For example, suppose that your class needs an instance of A in order to create an instance of B.  Furthermore, suppose our application needs to directly call methods on B but doesn't directly need to call any methods on A. We should have a factory method that internally creates an instance of A then an instance of B and returns B. In other words, our factories should create the objects we directly need instead of objects that are only used to create another object. This helps simplify consuming classes.  
