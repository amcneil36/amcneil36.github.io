...
Feature toggles allow us to turn on or off features without having to make a code change.<sup>2</sup> This is most commonly implemented by using a configuration file. The entry point of an application will then read from the configuration file to determine whether to have the feature turned on or off. You can have the feature turned off by default and then you can turn the feature on in your development and test environment while leaving it turned off in production. If the new feature is a button, having the feature turned off just means hiding the button. The code for the button is there but the button is hidden from the user so that the user experience is unaffected by the partially complete feature. 
...
Branch by abstraction is a technique for modifying existing functionality while still keeping trunk releasable.<sup>2</sup> The name of this technique is misleading as it is not creating a new branch. It entails
