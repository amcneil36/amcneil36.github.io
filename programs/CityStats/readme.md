...
## Weather and Demographics data calculated on the metro level (MetroSummary.csv)
MetroSummary.csv is a .csv file with weather and demographics data for all USA metros that have a population greater than 100k. All data is on the metro level. This means that you can see things like the average summer high for the metro. All numbers in this .csv (including population density) are calculated from using a weighted average except for the metro population column. The metro population is just a sum of the population of all cities in the metro. The weighted average is used for the rest of the columns so that a city of 10 people has less influence on the data than a city of 1,000,000 people. This is so the statistics are representative of a typical person living there. The "Predominant State" column is the state that contributes the most to the population of the metro. It is called "Predominant State" as opposed to "State" since some metros have cities in different states (ex: Kansas City has cities in Kansas and Missouri). The data for this .csv was calculated from using the data from CityStats.csv. MetroSummary.csv can be found here: <a href="https://amcneil36.github.io/programs/MetroStats/MetroSummary.csv" download>https://amcneil36.github.io/programs/MetroStats/MetroSummary.csv</a>

## Folder with one .csv per metro with data on each city within the metro (Metros folder)
This folder takes data from CityStats.csv and produces one .csv per metro that has a metro population greater than 100k. Unlike MetroSummary.csv which does calculations to produce data on the metro level, this one doesn't do any calculations. This one just takes data from CityStats.csv and separates it to different .csv's based on metro. This would be the same result as if you had opened up CityStats.csv and sorted the data by metro (with one exception being that this only picks the metros from CityStats.csv that have a metro population greater than 100k). This folder can be found here: [https://github.com/amcneil36/amcneil36.github.io/tree/master/programs/MetroStats/Metros](https://github.com/amcneil36/amcneil36.github.io/tree/master/programs/MetroStats/Metros)
